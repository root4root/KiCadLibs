<!--XSL style sheet to convert EESCHEMA XML Partlist Format to grouped CSV BOM Format
	...
	Functionality:
		Generation of NextPCB PCBA compatible BOM
    ...
    The command line is
        xsltproc -o "%O.csv" "FullPathToFile/bom2grouped_csv_nextpcb.xsl" "%I"
-->
<!--
    @package
    Generates a NextPCB PCBA service compatible BOM
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="/export">
	    <xsl:text>Designator, Quantity, Manufacturer Part Number, Manufacturer, Package/Footprint, Description, Procurement Type, Customer Note</xsl:text>
	    <!-- all table entries -->
	...
	<xsl:template match="components">
		...
		<xsl:text>","</xsl:text>
		<xsl:value-of select="fields/field[@name='MFR']"/>
		<xsl:text>","</xsl:text>
		<xsl:value-of select="fields/field[@name='MANUF']"/>
		<xsl:text>","</xsl:text>
		<xsl:value-of select="footprint"/>
		<xsl:text>","</xsl:text>
		<xsl:value-of select="value"/>
		<xsl:text>","</xsl:text>
		<xsl:text>","LCSC Part #: </xsl:text>
